...
void NAMES(Client &client, const std::vector<std::string> &params, std::map<std::string, Channel> &allChannels)
{
	...
	if (params.size() == 1)
	{
		const std::vector<std::string> channelNames = splitBycomma(params[0]);
		for (size_t i = 0; i < channelNames.size(); i++)
		{
			...
			if (
					(specific_chars.find_first_not_of(channelName[0]) != std::string::npos) && findChannelForServer(allChannels, &channelName[1]))
			{
